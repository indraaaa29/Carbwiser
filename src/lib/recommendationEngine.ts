...
import { TRANSPORT_FACTORS, ENERGY_FACTORS } from '../constants/carbonFactors';
...
function transportShare(p: UserProfile): number {
  // Car/EV users have higher transport share; walkers/cyclists very low
...
// Each entry is a function that receives the profile and returns the full
// Recommendation object (with a computed score).
type CandidateFactory = (p: UserProfile) => Recommendation;
